...
def edit_frame(frame_to_edit):
    ...
    edited_frame = cv2.cvtColor(frame_to_edit, cv2.COLOR_BGR2GRAY)
    return edited_frame


if __name__ == "__main__":
    # get figure from video
    print('Hello')
    cap = cv2.VideoCapture(0)
    cap.release()
    ret, frame = cap.read()

    while ret and cv2.waitKey(1) & 0xFF == 'q':
        cv2.inshow(frame, "Normal")           # display video
        edited_frame = edit_frame(frame)
        cv2.inshow(edited_frame, "Edited")    # display edited video
